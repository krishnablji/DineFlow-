...
const LoginPage = () => {
    ...
    if (isRegister) {
      ...
      if (res.success) {
        ...
        setTimeout(() => {
          ...
          else if (res.user.role === 'waiter') navigate('/kitchen');
          else navigate('/menu');
        }, 1000);
      } else {
  ...
  const handle1ClickDemo = async (demoRole) => {
    ...
    if (res.success) {
      ...
      else if (demoRole === 'waiter') navigate('/kitchen');
      else navigate('/menu');
    }
  };
      ...
      <div className="w-full max-w-md glass-panel-glow p-8 rounded-3xl border border-gold-500/30 bg-dark-800/90 text-slate-100 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="font-serif text-2xl font-bold tracking-tight text-slate-100">
            {isRegister ? 'Create DineFlow Account' : 'Welcome Back'}
          </h2>
          <p className="text-xs text-slate-400">
            {isRegister
              ? 'Register as customer, waiter, or restaurant manager'
              : 'Sign in to access your culinary workflow'}
          </p>
        </div>
        ...
        <div className="p-3.5 rounded-2xl bg-dark-900/80 border border-dark-700 space-y-2">
          <div className="flex items-center gap-1.5 text-[11px] font-bold text-gold-400 uppercase tracking-wider">
            ...
            <span>Zero-Friction 1-Click Demo Login:</span>
          </div>
          <div className="grid grid-cols-3 gap-2 pt-1">
            <button
              type="button"
              onClick={() => handle1ClickDemo('customer')}
              className="p-2 rounded-xl bg-dark-800 hover:bg-gold-500 hover:text-dark-900 border border-dark-600 text-[11px] font-semibold text-slate-300 transition-all flex flex-col items-center gap-1"
            >
              <span>🍔</span>
              <span>Customer</span>
            </button>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Chef / Manager / Guest Name"
              ...
              <div className="space-y-1">
                ...
                {role === 'waiter' && (
                  <p className="text-[10px] text-amber-400/90 pt-1">
                    * New waiter accounts are sent to the Manager Vetting Queue for approval before kitchen access.
                  </p>
                )}
              </div>
            </>
